# Arduino EUNO ESP32 局域网/物联网/远程控制服务器示例项目

## 简介

本项目通过 http 协议，实现了一个简单的 Arduino EUNO ESP32 开发板的，关于局域网/物联网/远程控制的，控制板载 LED 的服务器示例项目。由于仅使用不安全的 http 协议等因素，**请勿在不安全的网络环境中使用该项目代码**。
...
2. 通过命令行程序上传代码。linux系统下，通过`ls /dev/ttyUSB*`查询对应的串口。通过`ampy --port /dev/ttyUSBx put your_script.py`上传代码。串口`ttyUSBx`要确保是自己使用的，这里只是一个示范而已。
